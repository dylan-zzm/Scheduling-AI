...
import { extractImageUrls, parseTaskResult } from './image-result';
...
export function ImageGenerator({
  ...
  const [model, setModel] = useState(
    getDefaultImageModel('text-to-image', DEFAULT_IMAGE_PROVIDER)
  );
  ...
  const handleTabChange = (value: string) => {
    ...
    setActiveTab(tab);
    setModel(getDefaultImageModel(tab, provider));
  ...
  const handleProviderChange = (value: string) => {
    setProvider(value);
    setModel(getDefaultImageModel(activeTab, value));
  };
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={model} onValueChange={setModel}>
                      ...
                      <SelectContent>
                        {getAvailableImageModels(activeTab, provider).map(
                          (option) => (
                            <SelectItem key={option.value} value={option.value}>
                              {option.label}
                            </SelectItem>
